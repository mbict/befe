	...
	return api.Get(UrlBuilder("/accounts")).
		OnSuccess(
			api.Get( //this api is paginated by offset, so we need to visit all pages to get a full result
				UrlBuilder("/other?account_id={account_id}&size={size}",
				...
				OnSuccess(
					ResultMerger(). //merge the current resultset of the lookup back to the first result set
							Target("data.*.name"). //we merge the result into a new field called product_plans
							SubResult("$.name").
