...
func TestProgram(t *testing.T) {
	testcases := []struct {
		...
		{
			scenario:         "if chain executed correctly, we get the value of the context in the response body",
			method:           "GET",
			path:             "/foo",
			expectedCode:     200,
			expectedResponse: []byte(`got value from middleware via context : foobar`),
		},
	}
